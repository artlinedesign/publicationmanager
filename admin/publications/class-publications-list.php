...
class Publications_List extends WP_List_Table {
    ...
    function get_sortable_columns() {
        $sortable_columns = array(
            ...
            'seiten'     => array('seiten',false),     //true means it's already sorted
            'titel'     => array('titel',false),     //true means it's already sorted
            'datum'     => array('datum',false),     //true means it's already sorted
        );
    ...
    function prepare_items() {
         ...
         */
        $per_page     = $this->get_items_per_page( 'verlage_per_page', 10 );
        ...
        $sql = "SELECT {$wpdb->prefix}publicationmanager_authors.title, {$wpdb->prefix}publicationmanager_authors.firstname, {$wpdb->prefix}publicationmanager_authors.lastname, seiten, datum, titel, {$wpdb->prefix}publicationmanager_verlage.name AS verlag FROM {$wpdb->prefix}publicationmanager_publications LEFT JOIN {$wpdb->prefix}publicationmanager_authors ON author_id = {$wpdb->prefix}publicationmanager_authors.id LEFT JOIN {$wpdb->prefix}publicationmanager_verlage ON verlag_id = {$wpdb->prefix}publicationmanager_verlage.id";
...
//        $sql .= ' OFFSET ' . ( $page_number - 1 ) * $per_page;
        ...
        foreach($data as $i=>$d){
            $data[$i]['author'] = $d['title'] . ". " . $d['firstname'] . " " . $d['lastname'];
        }
